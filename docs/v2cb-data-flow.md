...
本项目不持久化微信读书数据；API Key 默认仅保存在页面会话中，用户也可以主动选择保存在当前浏览器：

```mermaid
flowchart LR
    D["浏览器 localStorage\n用户主动选择"] -.->|"恢复 API Key"| A["浏览器内存\n当前 API Key"]
    A -->|"同源请求"| B["本项目只读代理"]
    B -->|"Bearer Key"| C["微信读书 Agent Gateway"]
    C --> B --> A
```

- API Key 默认不进入持久化存储；开启“在此浏览器保存”后会以明文写入 `localStorage`，并在界面中明确提示风险。
- API Key 不进入项目数据库，取消保存或主动断开会清除浏览器中的副本。
